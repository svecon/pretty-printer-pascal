...
var keywords : array[1..18] of string = ('program', 'uses', 'var', 'const', 'type', 'begin', 'end', 'while', 'for', 'do', 'not', 'function', 'procedure', 'if', 'then', 'array', 'of', 'else');
...
{var dataTypes: array[1..13] of string = ('byte', 'shortint', 'word', 'integer', 'longint', 'real', 'single', 'double', 'extended', 'comp', 'bool', 'boolean', 'string');}
...
procedure outputNewLine(closeTag : boolean = true); forward;
...
procedure outputLine(text : string; printingHTML : boolean = false);
  begin
...
procedure outputNewLine(closeTag : boolean = true);
  begin
    if closeTag then
      begin
...
procedure runPrettyPrinter(inputSourceName : string = defaultSourceName; plainText : boolean = false);
  var whatFile : text;
  var c, pc : char;
  var sOut : string;
  begin
...
procedure varReplaceAndPrint(line : string);
  ...
  begin
    ...
    if 'output' = variableName then
      runPrettyPrinter(inputSourceName)
    else
      runError(777);
...
procedure outputTemplate(outputName : string = defaultOutputName; templateName : string = defaultTemplateName);
  var line : string;
  begin
